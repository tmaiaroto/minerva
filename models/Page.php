...
class Page extends \minerva\models\MinervaModel {
	...
	protected $_schema = array(
		'title' => array(
			'type' => 'string',
			'form' => array('label' => 'Title', 'wrap' => array('class' => 'minerva_title_input'))
		),
		'body' => array(
			'type' => 'string',
			'form' => array('type' => 'textarea', 'class' => 'tinymce')
		),
		'url' => array(
			'type' => 'string',
			'form' => array(
				'label' => 'Pretty URL',
				'help_text' => 'Set a specific pretty URL for this page (optionally overrides the default set from the title).',
		...
		'published' => array(
			'type' => 'boolean',
			'form' => array('type' => 'checkbox', 'position' => 'options')
		),
		'owner_id' => array(
			'type' => 'string',
			'form' => array('type' => 'hidden', 'label' => false)
		)
			// add options field?? to all models?? libraries can store various data within this
			// options field that contains array data.
			// it's useful...BUT, it won't have validation. it's still good to store simple data
			// that isn't harmful if left unvalidated or data that's set programatically so it's
			// known and doesn't need to be validated.
	);
	// Defined as normal and the library Page model's $validates is also defined as normal,
		...
		'read' => array(
			...
			'document' => array(
				array(
					...
					'message' => 'You are not allowed to see unpublished content.',
					'redirect' => array(
						'library' => 'minerva',
						'controller' => 'pages',
						'action' => 'index'
...
/* FILTERS GO HERE
 *
 * Any core filters must be set down here outside the class because of the class extension
 * by libraries.
