...
class UsersController extends \minerva\controllers\MinervaController {
	 ...
	 *  Plugins can apply filters within their User model class in order to run filters
	 *  for the delete.
	 ...
	 * Other libraries will need to replace this method and route registration depending
	 * on the desired functionality for the application.
	 */
	public function register() {
		// first, get all the data we need. this will set $document_type, $type, $modelClass,
		// and $display_name
		...
		if ($this->request->data) {
			...
			// set basic user, always hardcoded and set
			// IF this is the first user ever created, then they will be an administrator
			// TODO: make a wizard that will set this so there's no chance of some user registering
			// and becoming an admin
	...
	public function confirm($approval_code = null) {
		...
		// Save the confirmed field as true and set the approval_code to empty string just in case,
		// I know they are supposed to be unique strings but maybe that dart thrown from outerspace
		// will hit my house.
		$data = array('confirmed' => true, 'approval_code' => '');

		// If this was an e-mail change, take the new e-mail address and use it by setting
		// the email field equal to it and then get rid of the new_email field value
		...
		if ($record->save($data, array('validate' => false))) {
			...
			$message = 'Could not create the user record, please try again.';
			FlashMessage::write($message, array(), 'minerva_admin');
			// probably should redirect to a page where you can enter the code manually
			//or a retry or something. should notify the user to try again.
			$this->redirect(MINERVA_BASE_URL);
		}
	}
	...
	public function login() {
		...
		if (!Session::check('triedAuthRedirect', array('name' => 'minerva_cookie'))) {
			Session::write('triedAuthRedirect', 'false', array(
				'name' => 'minerva_cookie',
				'expires' => '+1 hour')
			);
		}
		...
		if ($user) {
			...
			if (Session::check('beforeAuthURL', array('name' => 'minerva_cookie'))) {
				...
				// 'triedAuthRedirect' so we don't end up in a redirect loop
				$triedAuthRedirect = Session::read('triedAuthRedirect', array(
					'name' => 'minerva_cookie'
				));
				if ($triedAuthRedirect == 'true') {
					...
					Session::write('triedAuthRedirect', 'true', array(
						'name' => 'minerva_cookie',
						'expires' => '+1 hour'
					));
				}
			...
			//$user_record = User::find('first', array(
			//	'conditions' => array(
			...
			if ($user_record) {
				$user_record->save(array(
					'last_login_ip' => $_SERVER['REMOTE_ADDR'],
					'last_login_time' => date('Y-m-d h:i:s')
				));
			}

			// only set a flash message if this is a login.
			// it could be a redirect from somewhere else that has restricted access
	...
	public function logout() {
		// get the redirects (again, call AFTER $this->getDocument()
		// because the $ModelClass will have changed)
		$action_redirects = $this->getRedirects();
	...
	public function update_password($url = null) {
		...
		if ($this->request->data) {
			...
			if ($this->request->data['password'] != $this->request->data['password_confirm']) {
				$message = 'You must confirm your password change by typing it again';
				$message .= ' in the confirm box.';
				return array('error' => true, 'response' => $message);
			}
			...
			if ($record->save($this->request->data)) {
				...
				return array('error' => false, 'response' => $message);
			} else {
				$message = 'Failed to update password, try again.';
				return array('error' => true, 'response' => $message);
			}
		} else {
			$message = 'You must pass the proper data to change your password and';
			$message .= ' you can\'t call this URL directly.';
			return array('error' => true, 'response' => $message);
		}
	}
}
