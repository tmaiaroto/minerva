...
class ThemeTest extends \lithium\test\Unit {
	...
	public function testSetRenderPaths() {
		...
		$result = Theme::setRenderPaths($this->request);
		$expected = LITHIUM_APP_PATH . '/views/minerva/_admin/layouts/{:layout}.{:type}.php';
		$this->assertEqual($expected, $result['layout'][0]);
		...
		$result = Theme::setRenderPaths($this->request);
		$expected = LITHIUM_APP_PATH . '/views/minerva/layouts/static/{:layout}.{:type}.php';
		$this->assertEqual($expected, $result['layout'][0]);
		...
		$result = Theme::setRenderPaths($this->request);
		$expected = LITHIUM_APP_PATH . '/views/minerva/_admin/layouts/static/{:layout}.{:type}.php';
		$this->assertEqual($expected, $result['layout'][0]);
		...
		$result = Theme::setRenderPaths($this->request);
		$expected = LITHIUM_APP_PATH . '/views/minerva/_plugin/minerva_blog/layouts/';
		$expected .= '{:layout}.{:type}.php';
		$this->assertEqual($expected, $result['layout'][0]);
		...
		$result = Theme::setRenderPaths($this->request);
		$expected = LITHIUM_APP_PATH;
		$expected .= '/views/minerva/_plugin/minerva_blog/layouts/static/{:layout}.{:type}.php';
		$this->assertEqual($expected, $result['layout'][0]);
	}
}
