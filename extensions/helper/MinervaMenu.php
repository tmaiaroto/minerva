...
class MinervaMenu extends Block {
	...
	public function request($url = null) {
		...
		$params = array('controller' => 'menus', 'action' => 'read', 'args' => $url);
		return $this->requestAction($params);
	}

	/**
	 * Shortcut helper method to render an admin menu.
	 * Works very similar to the Blocks helper's render_admin_block() method because it uses the
	 * same render() method.
	 *
	 * @param string $template The name of the template file located in minerva/views/menus/static/
	 * @param string $library The library from where to pull the menu (default is minerva,
	 *               falls back to app)
	...
	/**
	 * Shortcut helper method to render an admin menu.
	 * Works very similar to the Blocks helper's render_block() method because it uses the
	 * same render() method.
	 *
	 * @param string $template The name of the template file located in minerva/views/menus/static/
	 * @param string $library The library from where to pull the menu (default is minerva,
	 ...
	 * For now, this only goes two deep. In the future it should allow for more levels...
	 * Though there should be a limit.
	 *
	...
	public function static_menu($name = null, $options = array()) {
		...
		// because Menu::static_menu() should not be called provided there's a copy in cache)
		if (!empty($options['cache'])) {
			$menu = Cache::read('default', $cache_key);
		}

		// if the menu hasn't been set in cache or it was empty for some reason,
		// get a fresh copy of its data
		if (empty($menu)) {
		...
		if (is_array($menu)) {
			...
			foreach ($menu as $parent) {
				...
				$options = (
						...
						is_array($parent['options'])
					) ? $parent['options'] : array();
				$sub_items = (
						isset($parent['sub_items']) &&
						is_array($parent['sub_items'])
					) ? $parent['sub_items'] : array();

				if ($title && $url) {
					$position_class = ($i == 1) ? ' menu_first' : '';
					$position_class = ($i == $total) ? ' menu_last' : $position_class;
					$string .= "\t";
					$string .= '<li class="menu_item' . $position_class . '">';
					$string .= $this->_context->html->link($title, $url, $options);
					// sub menu items
