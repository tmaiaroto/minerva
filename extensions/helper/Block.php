...
* or render view templates from the views folder (by default located in: app/views/blocks/static). 
*
	...
	*/
	public function requestAction($options=array()) {
		...
		if(empty($options['library'])) {
			$class = '\app\controllers\\'.$controller_name.'Controller';
		} else {
			$class = '\app\libraries\\'.$options['library'].'\controllers\\'.$controller_name.'Controller'; 			
		}
