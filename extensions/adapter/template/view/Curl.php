 ...
 * It is meant to be a way to render external data instead of your conventional view templates from
 * local disk.
 * All of the options for cURL can be passed to take full advantage.
 *
 * Why not fopen/fread? Because cURL is much much faster and includes more options like
 * authenticaion and SSL. It should also be safer (even though since PHP 5.2.0 there's an
 * 'allow_url_include' setting set to off by default in php.ini).
 */
...
class Curl extends \lithium\template\view\Renderer {
	...
	public function render($template, $data = array(), array $options = array()) {
		$defaults = array(
			'url' => '/',
			'curl_options' => array(CURLOPT_HEADER => 0, CURLOPT_RETURNTRANSFER => 1)
		);
		$options += $defaults;
